...
**Ejemplo real**: PlayMigrate es riesgo mínimo porque no usa IA, solo OAuth y búsqueda por código.
...
**Ejemplo real**: El checkbox obligatorio de PlayMigrate antes de conectar Spotify: "He leído la Política de Privacidad y acepto que procese mis datos."
...
**Ejemplo real**: Cuando pulsa "Conectar con Spotify" y aparece la pantalla de Spotify que dice "PlayMigrate quiere acceder a tus playlists".
...
**Ejemplo real**: No usamos cookies en PlayMigrate. Los tokens viven en memoria de la sesión, no en cookies.
...
**Ejemplo real**: El botón "Eliminar mis datos y cerrar" de PlayMigrate. Un clic y todo se borra.
...
**Ejemplo real**: Es el destino de migración del MVP de PlayMigrate.
...
**Ejemplo real**: PlayMigrate NO requiere DPIA porque no es alto riesgo.
...
**Ejemplo real**: PlayMigrate debe cumplir RGPD porque se ofrece en la UE y trata datos personales (email, tokens, nombres de playlist).
...
**Ejemplo real**: PlayMigrate usa HTTPS (Vercel/Railway lo hacen por defecto). Todos los tokens viajan cifrados.
...
**Ejemplo real**: PlayMigrate solo lee playlists y canciones. No lee el historial de escucha, no lee preferencias, no lee perfil social.
...
**Ejemplo real**: El MVP de PlayMigrate migra playlists de Spotify a Deezer. No migra álbumes, artistas ni Liked Songs (eso es v2).
...
**Ejemplo real**: PlayMigrate usa OAuth 2.0 para que el usuario autorice a la app a leer sus playlists de Spotify sin compartir su contraseña.
...
**Ejemplo real**: Spotify recomienda PKCE para apps que pueden tener el código expuesto (como apps web sin backend seguro). PlayMigrate lo usa en Spotify y TIDAL.

### PlayMigrate
Nombre del proyecto. App web que migra playlists de Spotify a plataformas éticas (Deezer, TIDAL) con un clic.
**Ejemplo real**: playmigrate.com — María entra, conecta Spotify, elige playlists, conecta Deezer, migra. 3 minutos.

### Rate limit
Límite de peticiones que una API permite por unidad de tiempo. Si lo superas, te bloquea temporalmente.
**Ejemplo real**: Spotify permite ~30 peticiones/segundo, Deezer ~50. PlayMigrate usa tandas de 50 playlists con pausas para no superarlos.
...
**Ejemplo real**: PlayMigrate será responsive: funciona igual en el móvil de María que en su portátil.

### Scope (OAuth)
Permiso específico que una app pide al usuario. No es "dame todo", es "dame esto y solo esto".
**Ejemplo real**: PlayMigrate pide a Spotify el scope `playlist-read-private` (leer playlists privadas) pero NO pide `user-read-recently-played` (historial de escucha).
...
**Ejemplo real**: PlayMigrate usó este enfoque en el Paso 5 (spec) antes del Paso 6 (tecnología).
...
**Ejemplo real**: Estará disponible como destino de migración en v2 de PlayMigrate, una vez se confirme que el tier THIRD_PARTY cubre los endpoints de búsqueda.
...
**Ejemplo real**: El access_token de Spotify dura 1 hora. PlayMigrate lo renueva automáticamente con el refresh_token.
...
**Ejemplo real**: PlayMigrate apunta al nivel AA: contraste 4.5:1, navegación por teclado, textos alternativos en imágenes.
...
**Ejemplo real**: PlayMigrate solo guarda datos durante la sesión. Al cerrar o pulsar "Borrar mis datos", todo se elimina. No hay base de datos persistente.
